...
namespace EventOrganizerAPI.Services
{
    public class EmailServis
    {
        ...
        public string BaseUrl => _configuration["App:BaseUrl"]?.TrimEnd('/');
        public string FrontendBaseUrl => _configuration["App:FrontendBaseUrl"]?.TrimEnd('/');
        ...
        /// <summary>
        /// Salje email sa linkom za resetovanje lozinke korisniku, organizatoru ili dobavljacu
        /// </summary>
        public async Task PosaljiResetPasswordEmail(string email, string token)
        {
            // **Bitna izmena:** koristi FrontendBaseUrl za reset link (otvara React, ne backend)
            var resetLink = $"{FrontendBaseUrl}/reset-password?token={token}";
            var body = $"Za resetovanje šifre kliknite na sledeći link:\n{resetLink}";
            await PosaljiEmail(email, "Resetovanje šifre", body);
        }

        public async Task PosaljiKodZaReset(string email, string code)
        {
            var body = $"Vaš kod za resetovanje šifre je: {code}\nKod važi 10 minuta.";
            await PosaljiEmail(email, "Kod za resetovanje šifre", body);
        }
    }
}