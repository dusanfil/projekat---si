...
namespace EventOrganizerAPI.Services
{
    public class AuthServis : IAuthServis
    {
        ...
        private readonly ResetPasswordCodeServis _resetPasswordCodeServis;

        public AuthServis(
            IMongoDatabase db,
            ITokenServis tokenServis,
            EmailServis emailServis,
            ResetPasswordCodeServis resetPasswordCodeServis)
        {
            ...
            _resetPasswordCodeServis = resetPasswordCodeServis;
        }
        ...
        // ----------- ZABORAVLJENA ŠIFRA: SLANJE KODA -----------
        public async Task ForgotPasswordSendCode(string email)
        {
            object user = await _korisnici.Find(x => x.Email == email).FirstOrDefaultAsync();
            if (user == null)
                user = await _organizatori.Find(x => x.Email == email).FirstOrDefaultAsync();
            if (user == null)
                user = await _dobavljaci.Find(x => x.Email == email).FirstOrDefaultAsync();

            if (user == null)
                return; // Ne leakuj postojanje
            ...
            await _resetPasswordCodeServis.InsertAsync(resetCode);

            // Pošalji mejl
            await _emailServis.PosaljiKodZaReset(email, code);
        }

        // ----------- RESET ŠIFRE SA KODOM -----------
        public async Task<bool> ResetPasswordByCode(string email, string code, string newPassword)
        {
            var validCode = await _resetPasswordCodeServis.GetValidCode(email, code);
            if (validCode == null) return false;

            // Traži korisnika po emailu
            var korisnik = await _korisnici.Find(x => x.Email == email).FirstOrDefaultAsync();
            if (korisnik != null)
            {
                korisnik.Sifra = BCrypt.Net.BCrypt.HashPassword(newPassword);
                await _korisnici.ReplaceOneAsync(x => x.Id == korisnik.Id, korisnik);
                await _resetPasswordCodeServis.SetUsed(validCode.Id);
                return true;
            }
            var organizator = await _organizatori.Find(x => x.Email == email).FirstOrDefaultAsync();
            if (organizator != null)
            {
                organizator.Sifra = BCrypt.Net.BCrypt.HashPassword(newPassword);
                await _organizatori.ReplaceOneAsync(x => x.Id == organizator.Id, organizator);
                await _resetPasswordCodeServis.SetUsed(validCode.Id);
                return true;
            }
            var dobavljac = await _dobavljaci.Find(x => x.Email == email).FirstOrDefaultAsync();
            if (dobavljac != null)
            {
                dobavljac.Sifra = BCrypt.Net.BCrypt.HashPassword(newPassword);
                await _dobavljaci.ReplaceOneAsync(x => x.Id == dobavljac.Id, dobavljac);
                await _resetPasswordCodeServis.SetUsed(validCode.Id);
                return true;
            }
