...
namespace EventOrganizerAPI.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        // ----------- ZAHTEV ZA RESET KODA -----------

        [HttpPost("forgot-password")]
        public async Task<IActionResult> ForgotPassword([FromBody] ForgotPasswordRequestDto dto)
        {
            await _authServis.ForgotPasswordSendCode(dto.Email);
            return Ok();
        }

        // ----------- RESET ŠIFRE UNOSOM KODA -----------

        [HttpPost("reset-password-by-code")]
        public async Task<IActionResult> ResetPasswordByCode([FromBody] ResetPasswordByCodeRequestDto dto)
        {
            var ok = await _authServis.ResetPasswordByCode(dto.Email, dto.Code, dto.NewPassword);
            if (!ok) return BadRequest("Kod nije validan ili je istekao.");
            return Ok();
        }
    }
}