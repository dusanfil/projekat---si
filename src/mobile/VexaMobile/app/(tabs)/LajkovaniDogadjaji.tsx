...
import { Dogadjaj, Korisnik } from "@/types/dogadjaj";
...
export default function EventList() {
    ...
    const [events, setEvents] = useState<Dogadjaj[]>([]);
    const [loading, setLoading] = useState(true);
    const [userId, setUserId] = useState('');
    const [userIdLoading, setUserIdLoading] = useState(true);
    const [korisnik, setKorisnik] = useState<Korisnik | null>(null);
    const [korisnikLoading, setKorisnikLoading] = useState(true);
    const [token, setToken] = useState<string | null>('');
    const [refreshKey, setRefreshKey] = useState(0);
    useFocusEffect(
        useCallback(() => {
            setKorisnikLoading(true);
    ...
    useEffect(()=>{
            ...
            const ucitajKorisnika = async() =>{
                ...
                setKorisnikLoading(false);
            }
            ucitajKorisnika();
        },[userId, refreshKey])

    useEffect(()=>{
        if(korisnikLoading) return;
        const ucitajDogadjaje = async()=>{
            const dogadjajIds = korisnik?.omiljeniDogadjaji.map(k => k);
